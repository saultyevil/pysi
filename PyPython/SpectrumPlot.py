...
def spec_plot_inclinations(fname: str, scale: str = "logy", smooth: int = 5, frequency_space: bool = False,  **kwargs):
    """
    ...
    spectrum = read_spec(fname)
    inclinations = spec_inclinations(spectrum)
    n_inclinations = len(inclinations)
    nrows, ncols = subplot_dims(n_inclinations)

    fig, ax = plt.subplots(nrows, ncols, figsize=figsize, squeeze=False, **kwargs)
    ...
    for i in range(nrows):
        for j in range(ncols):
            if ii > len(inclinations) - 1:
                break
            rflux = spectrum[str(inclinations[ii])].values.astype(float)
            flux = smooth_spectrum(rflux, smooth)
            ax[i, j].plot(x, flux, label="i = {}".format(ii) + r"$^{\circ}$", **kwargs)
            if scale == "loglog" or scale == "logx":
                ax[i, j].set_xscale("log")
            if scale == "loglog" or scale == "logy":
                ax[i, j].set_yscale("log")
            ax[i, j].set_xlabel(r"Wavelength [$\AA$]", **kwargs)
            if frequency_space:
                ax[i, j].set_xlabel(r"Frequency [Hz]", **kwargs)
            ii += 1
