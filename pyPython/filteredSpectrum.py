...
dumped_photon_columns_dict_lres = {
    "Freq.": 0, "Weight": 1, "Spec.": 2, "Scat.": 3, "RScat.": 4, "Orig.": 5, "Res.": 6, "LineRes.": 7
}

dumped_photon_columns_dict_nres = {
...
def write_delay_dump_spectrum_to_file(
    ...
    try:
        ...
        header = get_spectrum_inclinations(full_spec)
    except IOError:
        ...
        if extract_nres[0] != UNFILTERED_SPECTRUM:
            ...
            for i in range(n_extract):
                ...
                if photon_nres[p] == extract_nres[i]:
                    ...
                    spectrum[freq_index, photon_spc_i[p]] += photon_weights[p]
                    break
                # Or if it's "belongs" to the nres we want and it's last interaction
                # was a continuum scatter, then extract
                elif photon_lnres[p] == extract_nres[i] and photon_nres[p] < 0:
                    freq_index = np.abs(spectrum[:, 0] - photon_freqs[p]).argmin()
                    spectrum[freq_index, photon_spc_i[p]] += photon_weights[p]
                    break
        else:
...
def renormalize_spectrum(
    ...
    #  TODO remind me why there's a 4 pi here.. F = L / 4pi R ?
    # I think it's because the photon weights are a luminosity
    ...
    for i in range(n_bins - 1):
        ...
        for j in range(n_spec):
            ...
            spectrum[i, j + 1] *= (freq ** 2 * 1e-8) / (dfreq * d_norm_cm * C)  # TODO check if this is correct
...
def construct_filtered_spectrum(
    ...
    extract_nres: tuple = (UNFILTERED_SPECTRUM,), d_norm_pc: float = 100, use_jit: bool = True) -> np.ndarray:
    """
    ...
    if use_jit:
        ...
        spectrum = jit_bin_photon_weights(
            spectrum, freq_min, freq_max, photon_freqs, photon_weights, photon_spec_index, photon_nres, photon_lnres,
            extract_nres
        )
    else:
    ...
    spectrum = renormalize_spectrum(spectrum, column_index_dict, spec_norm, d_norm_pc)
...
def create_filtered_spectrum(
    ...
    if mode_line_res:
        dumped_photon_columns_dict = copy.deepcopy(dumped_photon_columns_dict_lres)
    else:
        dumped_photon_columns_dict = copy.deepcopy(dumped_photon_columns_dict_nres)
    ...
    dumped_photons = read_delay_dump(root, dumped_photon_columns_dict, wd, mode_line_res)
    n_spec = int(np.max(dumped_photons[:, dumped_photon_columns_dict["Spec."]])) + 1

    if not freq_min:
        freq_min = np.min(dumped_photons[:, dumped_photon_columns_dict["Freq."]])
    if not freq_max:
        freq_max = np.max(dumped_photons[:, dumped_photon_columns_dict["Freq."]])
    ...
    spectrum = construct_filtered_spectrum(
        dumped_photons, spectrum, spec_cycle_norm, n_cores, dumped_photon_columns_dict, extract_nres=extract_nres,
        d_norm_pc=d_norm_pc, use_jit=use_jit
    )

    # Write out the spectrum to file

    spectrum = write_delay_dump_spectrum_to_file(root, wd, spectrum, extract_nres, n_spec, n_bins, d_norm_pc)
