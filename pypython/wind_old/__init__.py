        ...
        if self.nx == 0 or self.nz == 0:
            try:
                ...
                if len(self.header[1]) > LEN_WHEN_1D_MODEL:
                    self.nz = int(pypython.simulation.grid.get_parameter(self.pf, "Wind.dim.in.z_or_theta.direction"))
...
class Wind:
    ...
    def __init__(
        ...
        self.elements = ()
        self.spectra = ()
        self.wind = pypython.AttributeDict({})
    ...
    def create_masked_arrays(self):
        ...
        if DEBUG_MASK_CELL_SPEC:
            for cell_spec_type in self.spectra:
                self.wind[cell_spec_type].spectra = np.ma.masked_where(
                    self.wind["inwind"] != 0, self.wind[cell_spec_type].spectra
                )
    ...
    def get_wind_elements(self, elements_to_get=None, delim=None):
        ...
        for element in elements_to_get:
            ...
            self.wind[element] = pypython.AttributeDict({})
            ...
            for ion_type, ion_type_index_name in zip(["frac", "den"], ["fraction", "density"]):
                ...
                self.wind[element][ion_type_index_name] = pypython.AttributeDict({})
    ...
    def plot(self, variable_name, use_cell_coordinates=True, scale="loglog", log_variable=True, vmin=None, vmax=None):
        ...
        if self.coord_system == WindCoordSystem.spherical:
            ...
            fig, ax = plot._wind2d(
                n_points, m_points, variable, self.distance_units, self.coord_system, scale=scale, vmin=vmin, vmax=vmax
            )
