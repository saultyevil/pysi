...
def parse_arguments() -> Tuple[str, int, int, int, bool, str, str]:
    ...
    p.add_argument(
        "name", help="The name of the slurm file, i.e. name.slurm."
    )
    p.add_argument(
        "root", help="The root name of the model."
    )
    p.add_argument(
        "ncores", type=int, help="The number of CPUs to use."
    )
    p.add_argument(
        "thours", type=int, help="The number of hours of run time allowed."
    )
    p.add_argument(
        "tminutes", type=int, help="The number of minutes of additional run time allowed."
    )
    p.add_argument(
        "-f", "--flags", default="", help="Any flags to pass to the py_run.py Python running script."
    )
    p.add_argument(
        "-sc", "--split_cycle", action="store_true", default=False, help="Use the split cycle method for py_run.py"
    )
